...
├── graphics.py          # Создаёт графики из уже очищенных данных (запускается отдельно)
├── main.py              # Точка входа
...
1. Скачать датасет (можно один или несколько файлов) и положить `.parquet`-файлы в папку `DataSet/`
2. Запустить:
   ```bash
...
| `графики/*.png`             | Графики по содержимому датасета (при запуске raphics.py) |
